...
framework: HEOM and Lindblad agree on the qualitative ENAQT signal within
Markovian model limits, while the two-site Bloch-equation and $N$-site
Liouvillian implementations agree internally to numerical precision
($\Delta\eta < 10^{-15}$).
...
$N = 2$--$15$, 10,000 total measurements) reveals ENAQT in 93\,$\pm$\,4\%
of random configurations (Wilson 95\% CI: [89\%, 98\%])---a
universality result.
...
as ${\sim}2.1N$ before saturation; (iv) structural disorder is a co-resource
that amplifies ENAQT universally across ${\sim}93\%$ of random configurations
(Wilson 95\% CI: [89\%, 98\%]); and
...
Between 95\% and 100\% of disorder realizations (100 seeds per $N$) exhibit a
genuine interior ENAQT peak (Table~\ref{tab:disorder}).
The Wilson 95\% confidence interval for the lowest observed fraction
(95/100 seeds at $N=3$) spans $[89\%, 98\%]$, confirming that universality is
statistically robust rather than a finite-sample artefact~\cite{walschaers2013}.
...
The ${\sim}93\%$ universality result (Wilson 95\% CI: [89\%, 98\%]) is
particularly striking: ENAQT does not require tuning to a special disorder
configuration.
...
design, and its high ENAQT universality rate (${\sim}93\%$, Wilson 95\% CI:
[89\%, 98\%]) reflects the fact that almost any energy heterogeneity creates
some contrast.
...
\item \textbf{Validation:}
  The $N$-site Lindblad Liouvillian and two-site Bloch equations agree
  internally to machine precision ($|\Delta\eta| < 2.2\times10^{-16}$),
  confirming the implementation.
  Against HEOM, the Lindblad framework reproduces the ENAQT enhancement
  magnitude and optimal $\gps$ to within ${\lesssim}1\%$---quantitative
  agreement at the level of the Markovian approximation.
...
\item \textbf{ENAQT is universal and disorder-amplified:}
  ...
  ${\sim}93\%$ of configurations (Wilson 95\% CI: [89\%, 98\%]).
